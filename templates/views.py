...
def template_edit(request, template_id):
    ...
    return render(request, 'templates/template_form.html', {'template': template})
...
def batch_delete_templates(request):
    ...
    if request.method == 'POST':
        ...
        if not template_ids:
            return JsonResponse({'success': False, 'message': '请选择模板'})
        
        deleted_count = EmailTemplate.objects.filter(id__in=template_ids).update(is_active=False)
